<?php
/*
+---------------------------------------------------------------+
|        e107 website content management system Hungarian Language File
|        Released under the terms and conditions of the
|        GNU General Public License (http://gnu.org).
|        Last Modified: 2016/02/22 13:14:00
|
|        $Author: Yesszus $
+---------------------------------------------------------------+
*/
define("TPVLAN_1", "Ez a(z) <b>'".PREVIEWTHEMENAME."'</b> sablon előnézete.
Ez nem az oldalad sablonja, csak azt mutatja meg, hogyan nézne ki a weboldal ezzel a sablonnal.
<br />A sablon használatához lépj <a href='".e_ADMIN."theme.php'>a sablon kezelőbe</a> és válaszd a 'Beállítás az oldal témájaként' opciót.<br /><a href='".e_ADMIN."theme.php'>További témák előnézete</a>");
define("TPVLAN_2", "Sablon előnézete");
define("TPVLAN_3", "Az oldal témája megváltoztatva.");
define("TPVLAN_5", "Weboldal");
define("TPVLAN_6", "Kiadás dátuma");
define("TPVLAN_7", "Információ");
define("TPVLAN_9", "Sablon előnézete");
define("TPVLAN_10", "Beállítás az oldal témájaként");
define("TPVLAN_11", "Verzió");
define("TPVLAN_12", "Nincs elérhető előnézet");
define("TPVLAN_13", "Sablon feltöltése (.zip vagy .tar.gz formátum)");
define("TPVLAN_14", "Sablon feltöltése");
define("TPVLAN_15", "Az automatikus feltöltés nem lehetséges, mert a(z) ".e_THEME." könyvtár nem írható - adj rá írási jogot (CHMOD 777).");
define("TPVLAN_16", "Admin üzenet");
define("TPVLAN_17", "A fájl nem tűnik ténylegesen .zip vagy .tar archívnak.");
define("TPVLAN_18", "Hiba: a fájl kicsomagolása nem sikerült");
define("TPVLAN_19", "A sablon feltöltve és kicsomagolva.");
define("TPVLAN_20", "Az automatikus feltöltés és kicsomagolás nem lehetséges, mert a themes könyvtár nem írható - adj rá írási jogot (CHMOD 777).");
define("TPVLAN_21", "Jelenleg ez a beállított sablon");
define("TPVLAN_22", "a sablon több stíluslapot is tartalmaz");
define("TPVLAN_23", "alapértelmezett stíluslap");
define("TPVLAN_24", "nincs információ");
define("TPVLAN_26", "Sablon kezelő");
define("TPVLAN_27", "Válaszd ki a használandó stíluslapot");
define("TPVLAN_28", "be");
define("TPVLAN_29", "ki");
define("TPVLAN_30", "Sablon képek előtöltése:");
define("TPVLAN_31", "Jelenleg ez az adminisztrációs sablon");
define("TPVLAN_32", "Beállítás adminisztrációs sablonnak");
define("TPVLAN_33", "A weboldal sablonja");
define("TPVLAN_34", "Adminisztrációs sablon");
define("TPVLAN_35", "Beállítások mentése");
define("TPVLAN_36", "Admin üzenet");
define("TPVLAN_37", "A sablon beállítások elmentve");
define("TPVLAN_38", "Sablon feltöltése");
define("TPVLAN_39", "Elérhető sablonok");
define("TPVLAN_40", "Beállítás adminisztrációs sablonnak");
define("TPVLAN_41", "Válaszd ki a használandó admin elrendezési stílust");
define("TPVLAN_42", "Admin beállítások mentése");
define("TPVLAN_43", "Admin beállítások elmentve");
define("TPVLAN_46", "PCLZIP kicsomagolási hiba:");
define("TPVLAN_47", "PCLTAR kicsomagolási hiba:");
define("TPVLAN_48", "kód:");
define("TPVLAN_49", "Megfelelés");
define("TPVLAN_50", "Elrendezések");
define("TPVLAN_51", "Sablon cseréje");
define("TPVLAN_52", "Név");
define("TPVLAN_53", "Javasolt bővítmények");
define("TPVLAN_54", "Menü előbeállítások");
define("TPVLAN_55", "Alapértelmezett");
define("TPVLAN_56", "Megjelenítés szűrő");
define("TPVLAN_57", "Kompatibilitás");
define("TPVLAN_58", "Ez a sablon előtelepített példa tartalmat tartalmaz (úgy mint oldalak és menük) amelyeket használhat a weboldalán.");
define("TPVLAN_59", "Figyelj arra, hogy ezek a példa tartalmak [b]felülírják[/b] a saját jelenlegi tartalmat a következőkkel");
define("TPVLAN_60", "[x] bejegyzés a [y] táblában");
define("TPVLAN_61", "Szeretnéd [b]lecserélni[/b] a jelenlegi tartalmadat az alapértelmezett példa tartalommal, ha lehetséges?");
define("TPVLAN_62", "Sablon keresése");
define("TPVLAN_63", "Konvertálás");
define("TPVLAN_64", "Ez a varázsló létrehozza a sablon theme.xml meta fájlt.");
define("TPVLAN_65", "Mielőtt elkezdené:");
define("TPVLAN_66", "Győződjön meg róla, hogy a téma könyvtár írható-e.");
define("TPVLAN_67", "Válassza ki a téma mappát a kezdéshez.");
define("TPVLAN_68", "Válassza ki a téma mappát");
define("TPVLAN_69", "Letölthető");
define("TPVLAN_70", "Előnézet/Demó :");
define("TPVLAN_71", "Nincs meghatározva");
define("TPVLAN_72", "Oldalak beállítása amelyek automatikusan használják ezt az elrendezést.
1 oldal per sor.");
define("TPVLAN_73", "Menük aktiválása");
define("TPVLAN_74", "A következők aktiválása:");
define("TPVLAN_CONV_1", "1 lépés");
define("TPVLAN_CONV_2", "2 lépés");
define("TPVLAN_CONV_3", "A sablon neve. (Angol nyelven)");
define("TPVLAN_CONV_4", "Ha van nyelvi fájlt, adja meg LAN_XXX értékben a sablon nevét");
define("TPVLAN_CONV_5", "A sablon verziója. Formátum: x.x");
define("TPVLAN_CONV_6", "Létrehozás dátuma");
define("TPVLAN_CONV_7", "Kompatibilis az e107 verzióval");
define("TPVLAN_CONV_8", "Szerző");
define("TPVLAN_CONV_9", "Szerző weboldala");
define("TPVLAN_CONV_10", "A plugin egy rövid, egysoros ismertetése.
(!@#$%^&* karakterek nem engedélyezettek)
(Angol nyelven)");
define("TPVLAN_CONV_11", "A sablon kulcsszavak/címkék (Angol nyelven)");
define("TPVLAN_CONV_12", "A sablon teljes ismertetése (Angol nyelven)");
define("TPVLAN_CONV_13", "Milyen kategóriába tartozik ez a sablon?");
define("TPVLAN_CONV_14", "Engedélyezi választható opcióként a stíluslapot  a Theme Kezelőben.");
define("TPVLAN_CONV_15", "Adja meg a stíluslap nevét");
?>
